...
export default function SkillsCard({skills}) {
  return (
    <article className={"flex flex-col justify-center items-center gap-2 " +
      ...
      "bg-gray-50 p-8 " + styles.container}>
      <img className={styles.img} src={skills.img} alt={skills.alt}/>
      <h2 className={"font-extrabold text-2xl " + styles.title}>{skills.title}</h2>
      <hr className={"border block md:hidden " + styles.line}/>
      <div className={"flex justify-start items-start flex-wrap gap-2 " + styles.content}>
...
const SkillBox = ({children}) => {
  const color = useMemo(() => {
    return Math.floor(Math.random() * 72) * 5
  }, []);

  return (
    <div className={"font-bold text-lg border pl-1.5 pr-1.5 rounded-lg"}
